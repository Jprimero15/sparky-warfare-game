...
/**
 * Battle City-style grid combat with a full loop wired in: menu, waves,
 * scoring, power-up pickups (with the domain-expansion hero effect),
 * game over + restart, and Android touch controls alongside keyboard.
 *
 * Still open for you to extend: multiple arenas, sound, real sprite
 * art, more power-up types. Everything routes through GlowRenderer, so
 * new effects belong there.
 */
class GameScreen : Screen, InputAdapter() {
    ...
    private fun resetGame() {
        ...
        player = Tank(
            position = Vector2(WORLD_WIDTH / 2f, 40f),
            isPlayer = true,
    ...
    private fun nextWave() {
        ...
        for (i in 0 until count) {
            enemies.add(
                Tank(
                    position = Vector2(50f + i * (WORLD_WIDTH - 100f) / count, WORLD_HEIGHT - 60f),
                    angle = 270f,
    ...
    private fun updateEnemyAi(delta: Float) {
        // Minimal wander-and-shoot AI: drifts toward the player's column,
        // fires periodically. Replace with pathfinding/behavior states later.
        for (enemy in enemies) {
            val toPlayer = Vector2(player.position).sub(enemy.position)
            if (toPlayer.len() > 4f) {
                enemy.angle = toPlayer.angleDeg()
                val next = Vector2(enemy.position).mulAdd(toPlayer.nor(), enemy.speed * 0.5f * delta)
                if (!collidesWithWalls(next)) enemy.position.set(next)
            }
            if (enemy.canFire() && MathUtils.randomBoolean(0.01f)) {
                fireLaser(enemy)
            }
        }
    }
    ...
    private fun handleInput(delta: Float) {
        ...
        if (moveX != 0f || moveY != 0f) {
            val dir = Vector2(moveX, moveY)
            player.angle = dir.angleDeg()
            val next = Vector2(player.position).mulAdd(Vector2(dir).nor(), player.speed * delta)
            if (!collidesWithWalls(next)) player.position.set(next)
        }
    ...
    private fun collidesWithWalls(point: Vector2): Boolean =
        walls.any { it.alive && it.bounds.contains(point.x, point.y) }

    private fun outOfBounds(p: Vector2) = p.x < 0 || p.y < 0 || p.x > WORLD_WIDTH || p.y > WORLD_HEIGHT

    private fun fireLaser(tank: Tank) {
        val rad = Math.toRadians(tank.angle.toDouble())
        val dir = Vector2(Math.cos(rad).toFloat(), Math.sin(rad).toFloat())
        lasers.add(Laser(Vector2(tank.position), dir, Color(tank.color), firedByPlayer = tank.isPlayer))
    ...
    private fun checkLaserCollisions() {
        ...
        for (laser in lasers) {
            for (wall in walls) {
                if (wall.alive && wall.bounds.contains(laser.position.x, laser.position.y)) {
                    wall.hit()
            ...
            if (laser.firedByPlayer) {
                for (enemy in enemies) {
                    if (enemy.alive && enemy.position.dst(laser.position) < 14f) {
                        enemy.hit()
                        spawnBurst(laser.position, laser.color)
                        toRemove.add(laser)
            ...
            } else if (player.alive && player.position.dst(laser.position) < 14f) {
                player.hit()
                spawnBurst(laser.position, laser.color)
                toRemove.add(laser)
            }
    ...
    private fun spawnPowerUp() {
        ...
        powerUps.add(PowerUp(Vector2(WORLD_WIDTH / 2f, WORLD_HEIGHT / 2f), type))
    }
    ...
    private fun drawTouchControls() {
        ...
        // movement joystick base + knob (only while a finger is on it)
        if (joystick.active) {
        ...
        // fire button, bottom-right, always visible as a subtle target
    ...
    private fun drawGameOverOverlay() {
        ...
        font.draw(
            batch,
            "Tap anywhere or press SPACE to retry",
            Gdx.graphics.width / 2f - 220f,
    ...
    override fun touchDown(screenX: Int, screenY: Int, pointer: Int, button: Int): Boolean {
        ...
        val isLeftHalf = screenX < Gdx.graphics.width / 2
        if (isLeftHalf) {
